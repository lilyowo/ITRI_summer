...
export class ReportTitleComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.chartService.getReportTitleByReportId(this.reportId).subscribe(data => {
  ...
  exportAs(format: string): void {
    ...
    const simuTime = this.reportTitle[0]?.simuTime ? new Date(this.reportTitle[0].simuTime).toISOString() : 'unknown_time';
    const fileName = `Simulation_${reportName}_${simuTime}`;
    if (format === 'pdf') {
      // 利用window.print()方法來列印PDF
      window.print();
    }else if(format === 'word'){
      this.exportService.exportReport(this.reportId, format).subscribe(blob => {
        saveAs(blob, `${fileName}.docx`);
      });
    }
    else if(format==='excel'){
      this.exportService.exportReport(this.reportId, 'csv').subscribe(blob => {
